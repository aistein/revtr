...
def main():
    ...
    # CDF plots
    ind = np.arange(1,11)
    #fig, axs = plt.subplots(3,1, figsize=(7,14), sharex='col', sharey='row')
    fig, axs = plt.subplots(3,1, figsize=(10,14))
    fig.suptitle('Number of Hops from Chosen VP to Destination')
    for type_index, plot_type in enumerate(('bgp','asn','s24')):
        # gather counts
        counts_opt = multi_bar_graph_stats[type_index][0][1:11]
        counts_ingrfi = multi_bar_graph_stats[type_index][3][1:11]
        counts_ingrfo = multi_bar_graph_stats[type_index][4][1:11]
        counts_topk = multi_bar_graph_stats[type_index][1][1:11]
        counts_dest = multi_bar_graph_stats[type_index][2][1:11]
        counts_opp = multi_bar_graph_stats[type_index][5][1:11]
        # compute CDFs
        cdf_opt = np.cumsum( counts_opt )
        cdf_ingrfi = np.cumsum( counts_ingrfi )
        cdf_ingrfo = np.cumsum( counts_ingrfo )
        cdf_topk = np.cumsum( counts_topk )
        cdf_dest = np.cumsum( counts_dest )
        cdf_opp = np.cumsum( counts_opp )
        print(cdf_opp)
        print(ind)
        # plot the CDFs
        print("\tplotting")
        axs[type_index].plot( ind, cdf_opt/op_in_range, label='Optimal', color='r' )
        axs[type_index].plot( ind, cdf_ingrfi/op_in_range, label='Ingress Score - FI', color='g' )
        axs[type_index].plot( ind, cdf_ingrfo/op_in_range, label='Ingress Score - FO', color='b' )
        axs[type_index].plot( ind, cdf_opp/op_in_range, label='One Per Prefix (IS-FI)', color='y' )
        axs[type_index].plot( ind, cdf_topk/op_in_range, label='Top-10', color='c' )
        axs[type_index].plot( ind, cdf_dest/op_in_range, label='Destination Cover', color='m' )
        axs[type_index].grid(True)
        axs[type_index].legend(fontsize=8)
        tdef = 'BGP' if type_index == 0 else 'ASN' if type_index == 1 else 'S24'
        axs[type_index].set_title(tdef, fontsize=9)
    plt.savefig('hop_distribution_cdf.png')
    plt.clf()
